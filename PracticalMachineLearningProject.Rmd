...
A recent developent is wearable technology, which has the goal to integrate technology into the everyday life. Friction less wearable devices, such as watches, armand, belts, etc. can monitor our daily movement on multi-dimensional ways by employing accelerometers, gyrometers and other electronic sensors. These devices produce massive ammount of data which poses a challenge to be processed, encoding underlying information about the users of these devices.

The goal of this study is to use the data of Velloso, E. et al. priject. In Velloso’s program, a group of tester was required to perform 5 type of movements (here simplified as capital letter A ~ E), with sensor device positioned on 4 part of the body and a set of time sequence data is produced. The goal is to predict the manner in which they did the exercise, indictaed by the “classe” variable in the data set.

The study is struted as follows: 

1. Load the training and testing data
2. Conduct exploratory data analysis
3. Preprocess the data and eliminate not usefull variables
...
#exclude <- grepl('kurtosis|skewness|max|min|amplitude|var|avg|stddev|total', colnames)
...
Fortran code) for classification and regression. Random forests are very good for classification and regression. 
It uses multiple models for better performance. It also selects many samples in measuring variable importance. This approach can be used for model selection. It also works well when working with an extremely high number of candidate variables that need to be reduced.
...
In the next step we select the most important predictors by a) taking predictors by each classe factor having a higher accuracy than a given trashhold, b) by selecting the top 11 predictor orderd by mean accuracy, and c) by selecting the top 11 predictor orderd by mean gini measure. 
...
library(xtable)
print(xtable(accerrlist, caption = "List of model accuray and out-of-sample error"), type="html", comment=F)
```
...
library(gridExtra)
grid.arrange(cmPlot1, cmPlot2,cmPlot3, ncol=3)
```

Based on the accuray and confusion matrix results, we see that the tree model is perfoming the least with an accuray of `r round(accuracymod1,4)`. The k-fold corssvalidation model fit with the rain() method performes better with an accuracy of `r round(accuracymod1,4)`. The best fit, however, gives the fit with the randomForest() method with an accuray of `r round(accuracymod3,4)`. Therefore, we select the third model using the randomForest() method as our best model to predict the classe outcome for the test data set.
